...
## EPIC-006: フロントエンドUI推奨事項

> **参照タイミング:** EPIC-006「フロントエンド実装」の検索UI実装時

### セマンティック検索のUI説明

EPIC-005で実装する`/articles/search`エンドポイントは、SQLの`LIKE`ではなく**セマンティック検索（意味的類似度検索）** を使用する。
...
| 要素             | 推奨内容                                         |
| ---------------- | ------------------------------------------------ |
| プレースホルダー | `"怖い話"、"かわいい生物"など雰囲気で検索`       |
| ヘルプテキスト   | キーワード一致ではなく「意味」で検索する旨を説明 |
| 具体例提示       | ユーザーが検索方法を直感的に理解できる例を表示   |

### 対応可能なクエリ例

| クエリ例                       | 説明                               |
| ------------------------------ | ---------------------------------- |
| "ホラー系のscp"                | 意味的に「怖い」記事を返す         |
| "かわいい生物"                 | SCP-999など癒し系記事              |
...
- **検索**: pgvector コサイン類似度
- **多言語対応**: text-embedding-3-smallは多言語に対応

### 関連仕様

- [EPIC-005: バックエンドAPI](../specs/005-backend-api/005-backend-api.md) - セクション「6. フロントエンドへの推奨事項」

---

## Skill発動ルール違反の教訓（2026-02-27）

### 違反内容

specファイル（Story/Subtask）の作成を `/spec` Skillを経由せず、直接 Write ツールで作成した。

### 根本原因

- ユーザーから「specファイルも作っといて、完了ステータスでいいので」と言われた際、タスクの簡潔さから Skill 発動を省略してよいと誤判断した
- プランモードで /clarify 策定済みの計画があっても、spec作成は必ず /spec Skill経由が必要

### 正しい手順

1. `/clarify` で要件整理・計画策定
2. `/spec` Skill を発動してspecファイルを作成（**たとえ完了済みステータスでも**）
3. 実装は `/spec-workflow` Skill 経由

### 絶対ルール

- **specファイルの直接作成は禁止** — どんな状況でも `/spec` Skill を経由すること
- 「簡単だから」「完了済みだから」は省略の理由にならない
- CLAUDE.md の Skill自動発動テーブルは例外なく遵守する
