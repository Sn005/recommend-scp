...
export const createOnboardingRoutes = (
   ...
   *
   * Headers:
   * - Cache-Control: CDNキャッシュ可能
   */
  onboarding.get("/packs", (c) => {
   ...
   * @param packType - パック種別（horror, surreal, scientific, heartwarming, mystery）
   *
   * Response:
   * - 200 OK: 選択成功
  ...
  onboarding.post(
    "/select",
    zValidator("json", selectPackSchema, throwOnValidationError),
    async (c) => {
   ...
   * Response:
   * - 200 OK: 選択成功
   * - 400 Bad Request: バリデーションエラー（3件未満含む）
   * - 404 Not Found: visitorId未登録
   */
  onboarding.post(
    "/select/custom",
