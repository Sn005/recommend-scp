...
describe("recommend.csvファイル", () => {
  const csvPath = join(__dirname, "../testcases/recommend.csv");
  ...
  it("各行のIDがTC-006-02-{連番}形式である", () => {
    const csvContent = readFileSync(csvPath, "utf-8");
    const lines = csvContent.trim().split("\n").slice(1);
    const idPattern = /^TC-006-02-\d{3}$/;
  ...
  it("必須のtestIdがカバーされている", () => {
    const requiredTestIds = ["transition-card", "article-webview", "floating-favorite-button"];
    const csvContent = readFileSync(csvPath, "utf-8");
  ...
  it("API待ちのwaitForアクションにtimeoutが設定されている", () => {
    const csvContent = readFileSync(csvPath, "utf-8");
    expect(csvContent).toMatch(/"waitFor".*"transition-card".*"timeout"/);
  });
});
