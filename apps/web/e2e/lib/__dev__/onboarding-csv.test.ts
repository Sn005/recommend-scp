...
describe("onboarding.csvファイル", () => {
  const csvPath = join(__dirname, "../testcases/onboarding.csv");
  ...
  it("各行のIDがTC-006-01-{連番}形式である", () => {
    const csvContent = readFileSync(csvPath, "utf-8");
    const lines = csvContent.trim().split("\n").slice(1);
    const idPattern = /^TC-006-01-\d{3}$/;
  ...
  it("必須のtestIdがカバーされている", () => {
    const requiredTestIds = ["pack-selector", "pack-check"];
    const csvContent = readFileSync(csvPath, "utf-8");
