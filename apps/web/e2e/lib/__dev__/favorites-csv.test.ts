...
describe("favorites.csvファイル", () => {
  const csvPath = join(__dirname, "../testcases/favorites.csv");
  ...
  it("各行のIDがTC-006-03-{連番}形式である", () => {
    const csvContent = readFileSync(csvPath, "utf-8");
    const lines = csvContent.trim().split("\n").slice(1);
    const idPattern = /^TC-006-03-\d{3}$/;
  ...
  it("必須のtestIdがカバーされている", () => {
    const requiredTestIds = ["favorites-page", "favorite-list", "favorite-card"];
    const csvContent = readFileSync(csvPath, "utf-8");
