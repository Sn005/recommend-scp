...
- [x] getProfile で存在するvisitorのプロファイルを取得できる
- [x] getProfile で存在しないvisitorにnullを返す
- [x] saveProfile で新規プロファイルを保存できる
- [x] saveProfile で既存プロファイルを更新できる
- [x] getViewHistory で履歴をviewed_at降順で取得できる
- [x] addFeedback で同じ記事への既存フィードバックを上書きできる
- [x] getDislikedArticleIds でdislike記事IDのみ取得できる
- [x] getFavorites でadded_at降順で取得できる
- [x] snake_case ↔ camelCase 変換が正しく動作する
