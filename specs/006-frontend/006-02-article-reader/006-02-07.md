...
- [ ] WHEN ユーザーが「次へ」ボタン（ピルナビの→）をタップした際
      ...
      AND Dislikeフィードバックが記録される（既存挙動を維持）

### AC-6: 下部到達時のLike記録維持

- [ ] WHEN ユーザーが記事最下部に到達した際
      THEN 暗黙的Likeフィードバックが記録される（既存挙動を維持）
      AND 次の記事が自動的に表示される

### AC-7: バッファ先行取得失敗時のリトライ

- [ ] WHEN バックグラウンド取得が失敗した際
...
- 現在の記事のスクロール90%到達（既存postMessage検知）でコンテナをCSSスムーススクロール
...
sequenceDiagram
  ...
  U->>P: 記事をスクロール（iframe内postMessage）
...
| useFeedback         | 変更なし。Like/Dislike記録は既存通り |
...
- [ ] 初回読み込みでinitialCount=10件の記事が取得される
- [ ] DEFAULT_INITIAL_COUNTが10であること
- [ ] DEFAULT_LOAD_MORE_COUNTが5であること
- [ ] バッファ残数がprefetchThreshold以下で自動追加取得される
- [ ] バッファ残数がprefetchThresholdより多い場合は取得しない
- [ ] loadMore()で5件ずつ追加記事が取得される
- [ ] autoLoadLimitによる一時停止が発生しない（上限撤廃）
- [ ] isPaused/resumeAutoLoadが存在しない（削除確認）
- [ ] hasMoreがfalseの場合、先行取得は発生しない
- [ ] 取得中にloadMore()を呼んでも重複リクエストしない

### page.tsx 結合テスト

- [ ] 初期表示で最初の記事がWebViewに表示される
- [ ] スクロール90%到達で次の記事にスムーススクロールで遷移する
- [ ] スクロール遷移時にスクロール位置がジャンプしない（scrollTop値が連続的に変化する）
- [ ] 遷移完了後にcurrentIndexが+1される
- [ ] 遷移完了後に前の記事のiframeが破棄される（DOM上に2つ以下のiframe）
- [ ] 「次へ」ボタンで次の記事に遷移する（既存挙動維持）
- [ ] 「次へ」ボタン押下でDislikeが記録される（既存挙動維持）
- [ ] 下部到達時にLikeが記録される（既存挙動維持）
- [ ] スクロール遷移中に「次へ」ボタン連打で二重遷移しない
- [ ] バックグラウンド取得失敗時にエラー画面が表示されない

## デザイン準拠チェックリスト

**参照モック**: `mockups/header-6-minimal-2btn.html`

- [ ] 遷移アニメーションが滑らかでジャンプしない
- [ ] 既存のPillNav（♡ / →）が正常に表示・動作する
- [ ] 既存のProgressBarが正常に表示・動作する
- [ ] 既存のメニューボタン（☰）が正常に表示・動作する
- [ ] ローディングインジケータが不要に表示されない
