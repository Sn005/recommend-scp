...
(WebView)           (フローティングUI)
    └───────────────────┴───────────────────┘
            ...
            006-02-07 (推薦拡張・無限スクロール強化)
                    ↓
            006-02-08 (お気に入りボタンAPI連携) ← 005-10-03に依存
```
...
| AC-6: お気に入り追加       | 006-02-06, 006-02-08 | Favorite記録・API永続化                |
| AC-7: お気に入り解除       | 006-02-06, 006-02-08 | Favorite削除・API永続化                |
| AC-8: 推薦切れ対応         | 006-02-01            | EmptyState表示                         |
| AC-9: エラーハンドリング   | 006-02-01            | ErrorState表示                         |
| AC-3: 無限スクロール(強化) | 006-02-07            | 10件化・バッファ先行取得・スムース遷移 |
