...
- [x] explorationRate=0.2 の場合、約20%の確率でセレンディピティが選ばれる
- [x] 隣接領域記事は類似度0.4〜0.7の範囲から選ばれる
- [x] 未探索ジャンル記事はユーザーが読んでいないタグを持つ
- [x] 隣接領域と未探索ジャンルが適切な比率でマージされる
- [x] 返却されるarticleのsourceが"serendipity"に設定される
- [x] 除外IDリストが正しく適用される
- [x] 重複記事が除去される
