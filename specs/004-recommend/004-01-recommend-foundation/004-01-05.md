...
- [x] お気に入り記事のみの場合、そのEmbeddingがそのまま嗜好ベクトルになる
- [x] Like + 読了の混合で正しく重み付け平均が計算される
- [x] Dislike記事が負の重みで減算される
- [x] 履歴が空の場合、nullが返る
- [x] 記事のEmbeddingが取得できない場合、その記事はスキップされる
- [x] 全記事のEmbeddingが取得できない場合、nullが返る
