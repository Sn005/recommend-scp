...
- [x] タグが存在する記事IDでタグ配列を取得できる
- [x] 複数タグが存在する場合、全てのタグが配列に含まれる
- [x] タグが存在しない記事IDで空配列を返す
- [x] Supabaseエラー時に例外をスローする
- [x] エラーメッセージに記事IDと原因が含まれる

### 単体テスト: CompositeStorage

- [x] getProfile が localStorage に委譲される
- [x] saveProfile が localStorage に委譲される
- [x] getViewHistory が localStorage に委譲される
- [x] addViewHistory が localStorage に委譲される
- [x] getFeedback が localStorage に委譲される
- [x] getFeedbackByArticle が localStorage に委譲される
- [x] addFeedback が localStorage に委譲される
- [x] getRecommendationLog が localStorage に委譲される
- [x] addRecommendationLog が localStorage に委譲される
- [x] getDislikedArticleIds が localStorage に委譲される
- [x] getArticleTags が tagStorage に委譲される
