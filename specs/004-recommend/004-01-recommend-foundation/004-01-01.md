...
- [x] WHERE PreferenceStorageインターフェース
      THE SYSTEM SHALL 以下のメソッドを提供する: - getProfile / saveProfile（嗜好プロファイル）- getViewHistory / addViewHistory（閲覧履歴）- getFeedback / addFeedback（フィードバック）- getRecommendationLog / addRecommendationLog（推薦ログ）
...
- [x] PreferenceStorageインターフェースが正しくエクスポートされる
- [x] 全ての型定義がインポート可能である
- [x] StarterPackTypeが5種類 + customを含む
