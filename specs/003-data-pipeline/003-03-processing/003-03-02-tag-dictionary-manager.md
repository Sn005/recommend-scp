...
- [x] 辞書がDBから正しく取得される
- [x] キャッシュが正しく機能する
- [x] キャッシュの有効期限切れで再取得される
- [x] 正規値への変換が正しく行われる
- [x] 同義語からの正規化が正しく行われる
- [x] 大文字小文字を無視して正規化される
- [x] 未知のタグで警告が出力されnullが返される
- [x] プロンプト選択肢が正しく生成される

## 実装状況

- **status**: completed
- **実装ファイル**: `packages/shared/src/tagging/tag-dictionary-manager.ts`
- **テストファイル**: `packages/shared/src/tagging/tag-dictionary-manager.test.ts`
- **テスト結果**: 全20テストパス
